...
int main() {
    printf("Starting BTPN pattern generation...\n");
    
    FILE* file = fopen("BTPN.csv", "r");
    if (!file) {
        printf("Error: Cannot open BTPN.csv\n");
        return 1;
    }
    ...
    // Generate SQL file
    FILE* sql_file = fopen("master_customer_btp_pattern_BTPN.sql", "w");
    if (!sql_file) {
        printf("Error: Cannot create SQL file\n");
        return 1;
    }
    
    fprintf(sql_file, "-- BTPN Customer BTP Patterns\n");
    ...
    // Print summary
    printf("\n=== BTPN PATTERN GENERATION COMPLETE ===\n");
    printf("Total patterns generated: %d\n", pattern_count);
    printf("Total unique customers: %d\n", customer_total_count);
    printf("SQL file created: master_customer_btp_pattern_BTPN.sql\n\n");
