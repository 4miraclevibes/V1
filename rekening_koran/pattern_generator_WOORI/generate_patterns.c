...
// Format: "KR OTOMATIS LLG-WOORI SAUDARASIA [CUSTOMER_WORD1] [CUSTOMER_WORD2] [ADDITIONAL_INFO]"
...
int main() {
    printf("Starting WOORI pattern generation...\n");
    
    FILE* file = fopen("WOORI.csv", "r");
    if (!file) {
        printf("Error: Cannot open WOORI.csv\n");
        return 1;
    }
    ...
    // Generate SQL file
    FILE* sql_file = fopen("master_customer_btp_pattern_WOORI.sql", "w");
    if (!sql_file) {
        printf("Error: Cannot create SQL file\n");
        return 1;
    }
    
    fprintf(sql_file, "-- WOORI Customer BTP Patterns\n");
    fprintf(sql_file, "-- Generated from WOORI.csv\n");
    ...
    for (int i = 0; i < pattern_count; i++) {
        ...
        fprintf(sql_file, "('%s', '%s', \'WOORI\', %d, %d, %.2f, %d)",
