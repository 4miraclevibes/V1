...
// Function to extract customer name from WOORI description
// Format: "KR OTOMATIS LLG-WOORI SAUDARASIAESIA [CUSTOMER_WORD1] [CUSTOMER_WORD2] [ADDITIONAL_INFO]"
...
int load_patterns() {
    FILE* file = fopen("master_customer_btp_pattern_WOORI.sql", "r");
    if (!file) {
        printf("Error: Cannot open master_customer_btp_pattern_WOORI.sql\n");
        return 0;
    }
...
void manual_test() {
    ...
    printf("\n=== MANUAL TEST WOORI PATTERN ===\n");
    printf("Masukkan description WOORI (format: KR OTOMATIS LLG-WOORI SAUDARASIAESIA ...):\n");
    printf("Contoh: KR OTOMATIS LLG-WOORI SAUDARASIAESIA EMPU ANEKA PANGAN, GREENFIELDS\n");
    ...
    if (fgets(description, sizeof(description), stdin)) {
        ...
        if (extract_customer_name(description, customer_name)) {
            ...
            printf("Format yang benar: KR OTOMATIS LLG-WOORI SAUDARASIAESIA [WORD1] [WORD2] [ADDITIONAL_INFO]\n");
        }
    }
}
...
int main() {
    printf("WOORI BTP Pattern Tester\n");
    printf("======================\n");
    
    if (!load_patterns()) {
        printf("Failed to load patterns. Make sure master_customer_btp_pattern_WOORI.sql exists.\n");
        return 1;
    }
    
    printf("Loaded %d patterns from master_customer_btp_pattern_WOORI.sql\n", pattern_count);
