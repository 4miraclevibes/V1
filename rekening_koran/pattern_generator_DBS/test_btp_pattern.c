...
// Format: "KR OTOMATIS LLG-DBS INDONESIAESIA [CUSTOMER_WORD1] [CUSTOMER_WORD2] [ADDITIONAL_INFO]"
...
int load_patterns() {
    FILE* file = fopen("master_customer_btp_pattern_DBS.sql", "r");
    if (!file) {
        printf("Error: Cannot open master_customer_btp_pattern_DBS.sql\n");
        return 0;
    }
...
void manual_test() {
    ...
    printf("\n=== MANUAL TEST DBS PATTERN ===\n");
    printf("Masukkan description DBS (format: KR OTOMATIS LLG-DBS INDONESIAESIA ...):\n");
    printf("Contoh: KR OTOMATIS LLG-DBS INDONESIAESIA EMPU ANEKA PANGAN, GREENFIELDS\n");
    ...
    if (fgets(description, sizeof(description), stdin)) {
        ...
        if (extract_customer_name(description, customer_name)) {
            ...
            printf("Format yang benar: KR OTOMATIS LLG-DBS INDONESIAESIA [WORD1] [WORD2] [ADDITIONAL_INFO]\n");
        }
    }
}
...
int main() {
    printf("DBS BTP Pattern Tester\n");
    printf("======================\n");
    
    if (!load_patterns()) {
        printf("Failed to load patterns. Make sure master_customer_btp_pattern_DBS.sql exists.\n");
        return 1;
    }
    
    printf("Loaded %d patterns from master_customer_btp_pattern_DBS.sql\n", pattern_count);
