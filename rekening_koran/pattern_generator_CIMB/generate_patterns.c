...
int main() {
    printf("Starting CIMB pattern generation...\n");
    
    FILE* file = fopen("CMIB.csv", "r");
    if (!file) {
        printf("Error: Cannot open CMIB.csv\n");
        return 1;
    }
    ...
    // Generate SQL file
    FILE* sql_file = fopen("master_customer_btp_pattern_CIMB.sql", "w");
    if (!sql_file) {
        printf("Error: Cannot create SQL file\n");
        return 1;
    }
    
    fprintf(sql_file, "-- CIMB NIAGA Customer BTP Patterns\n");
    fprintf(sql_file, "-- Generated from CMIB.csv\n");
    ...
    for (int i = 0; i < pattern_count; i++) {
        ...
        fprintf(sql_file, "('%s', '%s', \'CIMB\', %d, %d, %.2f, %d)",
    ...
    // Print summary
    printf("\n=== CIMB PATTERN GENERATION COMPLETE ===\n");
    printf("Total patterns generated: %d\n", pattern_count);
    printf("Total unique customers: %d\n", customer_total_count);
    printf("SQL file created: master_customer_btp_pattern_CIMB.sql\n\n");
