...
// Function to extract customer name from CIMB description
// Format: "KR OTOMATIS LLG-CIMB NIAGA [CUSTOMER_WORD1] [CUSTOMER_WORD2] [ADDITIONAL_INFO]"
...
int load_patterns() {
    FILE* file = fopen("master_customer_btp_pattern_CIMB.sql", "r");
    if (!file) {
        printf("Error: Cannot open master_customer_btp_pattern_CIMB.sql\n");
        return 0;
    }
...
void manual_test() {
    ...
    printf("\n=== MANUAL TEST CIMB PATTERN ===\n");
    printf("Masukkan description CIMB (format: KR OTOMATIS LLG-CIMB NIAGA ...):\n");
    printf("Contoh: KR OTOMATIS LLG-CIMB NIAGA CAHAYA SAKTI 0000006726 A000134 62 112923 B H550\n");
    ...
    if (fgets(description, sizeof(description), stdin)) {
        ...
        if (extract_customer_name(description, customer_name)) {
            ...
            printf("Format yang benar: KR OTOMATIS LLG-CIMB NIAGA [WORD1] [WORD2] [ADDITIONAL_INFO]\n");
        }
    }
}
...
int main() {
    printf("CIMB BTP Pattern Tester\n");
    printf("=======================\n");
    
    if (!load_patterns()) {
        printf("Failed to load patterns. Make sure master_customer_btp_pattern_CIMB.sql exists.\n");
        return 1;
    }
    
    printf("Loaded %d patterns from master_customer_btp_pattern_CIMB.sql\n", pattern_count);
