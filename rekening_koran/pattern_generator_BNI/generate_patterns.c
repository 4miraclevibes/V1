...
int main() {
    printf("Starting BNI pattern generation...\n");
    
    FILE* file = fopen("BNI.csv", "r");
    if (!file) {
        printf("Error: Cannot open BNI.csv\n");
        return 1;
    }
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        // Skip lines where BTP is not numeric
        if (!isdigit(btp[0])) {
            continue;
        }
    ...
    // Generate SQL file
    FILE* sql_file = fopen("master_customer_btp_pattern_BNI.sql", "w");
    if (!sql_file) {
        printf("Error: Cannot create SQL file\n");
        return 1;
    }
    
    fprintf(sql_file, "-- BNI Customer BTP Patterns\n");
    fprintf(sql_file, "-- Generated from BNI.csv\n");
    ...
    for (int i = 0; i < pattern_count; i++) {
        ...
        fprintf(sql_file, "('%s', '%s', \'BNI\', %d, %d, %.2f, %d)",
    ...
    // Print summary
    printf("\n=== BNI PATTERN GENERATION COMPLETE ===\n");
    printf("Total patterns generated: %d\n", pattern_count);
    printf("Total unique customers: %d\n", customer_total_count);
    printf("SQL file created: master_customer_btp_pattern_BNI.sql\n\n");
