...
// Format: "KR OTOMATIS LLG-MUAMALAT INDON [CUSTOMER_WORD1] [CUSTOMER_WORD2] [ADDITIONAL_INFO]"
...
int main() {
    printf("Starting MUAMALAT pattern generation...\n");
    
    FILE* file = fopen("MUAMALAT.csv", "r");
    if (!file) {
        printf("Error: Cannot open MUAMALAT.csv\n");
        return 1;
    }
    ...
    printf("Processing MUAMALAT data...\n");
    ...
    // Generate SQL file
    FILE* sql_file = fopen("master_customer_btp_pattern_MUAMALAT.sql", "w");
    if (!sql_file) {
        printf("Error: Cannot create SQL file\n");
        return 1;
    }
    
    fprintf(sql_file, "-- MUAMALAT Customer BTP Patterns\n");
    fprintf(sql_file, "-- Generated from MUAMALAT.csv\n");
    ...
    for (int i = 0; i < pattern_count; i++) {
        ...
        fprintf(sql_file, "('%s', '%s', \'MUAMALAT\', %d, %d, %.2f, %d)",
