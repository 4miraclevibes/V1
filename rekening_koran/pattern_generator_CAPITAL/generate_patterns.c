...
// Format: "KR OTOMATIS LLG-CAPITAL INDONESIA [CUSTOMER_WORD1] [CUSTOMER_WORD2] [ADDITIONAL_INFO]"
...
int main() {
    printf("Starting CAPITAL pattern generation...\n");
    
    FILE* file = fopen("CAPITAL.csv", "r");
    if (!file) {
        printf("Error: Cannot open CAPITAL.csv\n");
        return 1;
    }
    ...
    printf("Processing CAPITAL data...\n");
    ...
    // Generate SQL file
    FILE* sql_file = fopen("master_customer_btp_pattern_CAPITAL.sql", "w");
    if (!sql_file) {
        printf("Error: Cannot create SQL file\n");
        return 1;
    }
    
    fprintf(sql_file, "-- CAPITAL Customer BTP Patterns\n");
    fprintf(sql_file, "-- Generated from CAPITAL.csv\n");
    ...
    for (int i = 0; i < pattern_count; i++) {
        ...
        fprintf(sql_file, "('%s', '%s', \'CAPITAL\', %d, %d, %.2f, %d)",
