...
// Function to extract customer name from CAPITAL description
// Format: "KR OTOMATIS LLG-CAPITAL INDONESIAESIA [CUSTOMER_WORD1] [CUSTOMER_WORD2] [ADDITIONAL_INFO]"
...
int load_patterns() {
    FILE* file = fopen("master_customer_btp_pattern_CAPITAL.sql", "r");
    if (!file) {
        printf("Error: Cannot open master_customer_btp_pattern_CAPITAL.sql\n");
        return 0;
    }
...
void manual_test() {
    ...
    printf("\n=== MANUAL TEST CAPITAL PATTERN ===\n");
    printf("Masukkan description CAPITAL (format: KR OTOMATIS LLG-CAPITAL INDONESIAESIA ...):\n");
    printf("Contoh: KR OTOMATIS LLG-CAPITAL INDONESIAESIA EMPU ANEKA PANGAN, GREENFIELDS\n");
    ...
    if (fgets(description, sizeof(description), stdin)) {
        ...
        if (extract_customer_name(description, customer_name)) {
            ...
            // Find all BTPs for this customer
            CustomerPattern found_patterns[100];
            int found_count = 0;
            
            if (find_all_btps(customer_name, found_patterns, &found_count)) {
                printf("\n=== FOUND %d BTP(s) FOR CUSTOMER: %s ===\n", found_count, customer_name);
                
                // Find the latest one (highest last_line_number)
                int latest_index = 0;
                for (int i = 1; i < found_count; i++) {
                    if (found_patterns[i].last_line_number > found_patterns[latest_index].last_line_number) {
                        latest_index = i;
                    }
                }
                ...
                for (int i = 0; i < found_count; i++) {
                    ...
                    printf("Customer Name       : %s\n", found_patterns[i].customer_name);
                    printf("BTP                 : %s\n", found_patterns[i].btp);
                    printf("Transaction Count   : %d (dengan BTP ini)\n", found_patterns[i].count);
                    printf("Total Transactions  : %d (total customer)\n", found_patterns[i].total_transactions);
                    printf("Match Percentage    : %.2f%% (BTP ini / total)\n", found_patterns[i].match_percentage);
                ...
                // Summary
                printf("\n📊 SUMMARY:\n");
                printf("Total unique BTPs for this customer: %d\n", found_count);
                printf("Recommended BTP (latest): %s\n", found_patterns[latest_index].btp);
            ...
            printf("Format yang benar: KR OTOMATIS LLG-CAPITAL INDONESIAESIA [WORD1] [WORD2] [ADDITIONAL_INFO]\n");
        }
    }
}
...
int main() {
    printf("CAPITAL BTP Pattern Tester\n");
    printf("======================\n");
    
    if (!load_patterns()) {
        printf("Failed to load patterns. Make sure master_customer_btp_pattern_CAPITAL.sql exists.\n");
        return 1;
    }
    
    printf("Loaded %d patterns from master_customer_btp_pattern_CAPITAL.sql\n", pattern_count);
