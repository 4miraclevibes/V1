...
int main() {
    printf("Starting MAYBANK pattern generation...\n");
    
    FILE* file = fopen("MAYBANK.csv", "r");
    if (!file) {
        printf("Error: Cannot open MAYBANK.csv\n");
        return 1;
    }
    ...
    printf("Processing MAYBANK data...\n");
    ...
    // Generate SQL file
    FILE* sql_file = fopen("master_customer_btp_pattern_MAYBANK.sql", "w");
    if (!sql_file) {
        printf("Error: Cannot create SQL file\n");
        return 1;
    }
    
    fprintf(sql_file, "-- MAYBANK Customer BTP Patterns\n");
    fprintf(sql_file, "-- Generated from MAYBANK.csv\n");
    ...
    for (int i = 0; i < pattern_count; i++) {
        ...
        fprintf(sql_file, "('%s', '%s', \'MAYBANK\', %d, %d, %.2f, %d)",
