...
// Function to extract customer name from MAYBANK description
// Format: "KR OTOMATIS LLG-MAYBANK [CUSTOMER_WORD1] [CUSTOMER_WORD2] [ADDITIONAL_INFO]"
...
int load_patterns() {
    FILE* file = fopen("master_customer_btp_pattern_MAYBANK.sql", "r");
    if (!file) {
        printf("Error: Cannot open master_customer_btp_pattern_MAYBANK.sql\n");
        return 0;
    }
...
void manual_test() {
    ...
    printf("\n=== MANUAL TEST MAYBANK PATTERN ===\n");
    printf("Masukkan description MAYBANK (format: KR OTOMATIS LLG-MAYBANK ...):\n");
    printf("Contoh: KR OTOMATIS LLG-MAYBANK AEON INDONESIA, PT\n");
    ...
    if (fgets(description, sizeof(description), stdin)) {
        ...
        if (extract_customer_name(description, customer_name)) {
            ...
            printf("Format yang benar: KR OTOMATIS LLG-MAYBANK [WORD1] [WORD2] [ADDITIONAL_INFO]\n");
        }
    }
}
...
int main() {
    printf("MAYBANK BTP Pattern Tester\n");
    printf("==========================\n");
    
    if (!load_patterns()) {
        printf("Failed to load patterns. Make sure master_customer_btp_pattern_MAYBANK.sql exists.\n");
        return 1;
    }
    
    printf("Loaded %d patterns from master_customer_btp_pattern_MAYBANK.sql\n", pattern_count);
