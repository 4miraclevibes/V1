...
// Function to extract customer name from UOB description
// Format: "KR OTOMATIS LLG-UOB INDONESIA [CUSTOMER_WORD1] [CUSTOMER_WORD2] [ADDITIONAL_INFO]"
...
int load_patterns() {
    FILE* file = fopen("master_customer_btp_pattern_UOB.sql", "r");
    if (!file) {
        printf("Error: Cannot open master_customer_btp_pattern_UOB.sql\n");
        return 0;
    }
...
void manual_test() {
    ...
    printf("\n=== MANUAL TEST UOB PATTERN ===\n");
    printf("Masukkan description UOB (format: KR OTOMATIS LLG-UOB INDONESIA ...):\n");
    printf("Contoh: KR OTOMATIS LLG-UOB INDONESIA EMPU ANEKA PANGAN, GREENFIELDS\n");
    ...
    if (fgets(description, sizeof(description), stdin)) {
        ...
        if (extract_customer_name(description, customer_name)) {
            ...
            printf("Format yang benar: KR OTOMATIS LLG-UOB INDONESIA [WORD1] [WORD2] [ADDITIONAL_INFO]\n");
        }
    }
}
...
int main() {
    printf("UOB BTP Pattern Tester\n");
    printf("======================\n");
    
    if (!load_patterns()) {
        printf("Failed to load patterns. Make sure master_customer_btp_pattern_UOB.sql exists.\n");
        return 1;
    }
    
    printf("Loaded %d patterns from master_customer_btp_pattern_UOB.sql\n", pattern_count);
