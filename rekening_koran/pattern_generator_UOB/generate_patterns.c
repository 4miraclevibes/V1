...
int main() {
    printf("Starting UOB pattern generation...\n");
    
    FILE* file = fopen("UOB.csv", "r");
    if (!file) {
        printf("Error: Cannot open UOB.csv\n");
        return 1;
    }
    ...
    // Generate SQL file
    FILE* sql_file = fopen("master_customer_btp_pattern_UOB.sql", "w");
    if (!sql_file) {
        printf("Error: Cannot create SQL file\n");
        return 1;
    }
    
    fprintf(sql_file, "-- UOB Customer BTP Patterns\n");
    fprintf(sql_file, "-- Generated from UOB.csv\n");
    ...
    for (int i = 0; i < pattern_count; i++) {
        ...
        fprintf(sql_file, "('%s', '%s', \'UOB\', %d, %d, %.2f, %d)",
