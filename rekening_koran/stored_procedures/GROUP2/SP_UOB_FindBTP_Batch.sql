...
--
-- Description:
--   Find BTP (Bill To Party) for UOB bank transactions (batch processing)
--   
-- Extraction Logic:
--   Group 2: Array[4] + Array[5] (Smart PT/CV)
--   Format: "KR OTOMATIS LLG-UOB INDONESIA [WORD1] [WORD2] [WORD3]..."
--   
...
CREATE OR ALTER PROCEDURE [dbo].[SP_UOB_FindBTP_Batch]
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            SELECT 
                ...
                ROW_NUMBER() OVER (
                    ORDER BY 
                        CASE WHEN category = 'UOB' THEN 1 ELSE 2 END,
            ...
            WHERE customer_name = @CustomerName
              AND (category = 'UOB' OR category = 'NEW')
            ORDER BY 
                CASE WHEN category = 'UOB' THEN 1 ELSE 2 END,
                match_percentage DESC,
                last_line_number DESC,
    ...
    PRINT '═══════════════════════════════════════════════════════════════════════';
    PRINT 'SP_UOB_FindBTP_Batch - Processing Complete';
    PRINT '═══════════════════════════════════════════════════════════════════════';
...
PRINT '✅ SP_UOB_FindBTP_Batch created successfully';
PRINT '   Bank: UOB';
