...
--
-- Description:
--   Find BTP (Bill To Party) for OCBC bank transactions (batch processing)
--   
-- Extraction Logic:
--   Group 2: Array[4] + Array[5] (Smart PT/CV)
--   Format: "KR OTOMATIS LLG-OCBC NISP [WORD1] [WORD2] [WORD3]..."
--   
--   Special Word: "NISP" at Array[3] (automatically skipped)
--   
--   Extraction:
--     • Array[4] + Array[5] (default: 2 words)
--     • If Array[4] = "PT" or "CV" → Array[4] + Array[5] + Array[6] (3 words)
--
-- Pattern Count: 6 patterns in master data
--
-- Parameters:
...
CREATE OR ALTER PROCEDURE [dbo].[SP_OCBC_FindBTP_Batch]
...
BEGIN
    ...
    BEGIN
        ...
        -- Special word "NISP" at Array[3] is skipped
        -- ═══════════════════════════════════════════════════════════════════
        ...
        BEGIN
            ...
            SELECT 
                ...
                ROW_NUMBER() OVER (
                    ORDER BY 
                        CASE WHEN category = 'OCBC' THEN 1 ELSE 2 END,
            ...
            WHERE customer_name = @CustomerName
              AND (category = 'OCBC' OR category = 'NEW')
            ORDER BY 
                CASE WHEN category = 'OCBC' THEN 1 ELSE 2 END,
                match_percentage DESC,
                last_line_number DESC,
    ...
    PRINT '═══════════════════════════════════════════════════════════════════════';
    PRINT 'SP_OCBC_FindBTP_Batch - Processing Complete';
    PRINT '═══════════════════════════════════════════════════════════════════════';
...
PRINT '✅ SP_OCBC_FindBTP_Batch created successfully';
PRINT '   Bank: OCBC';
PRINT '   Group: 2 (Array[4] + Array[5])';
PRINT '   Special Word: NISP';
PRINT '   Patterns in master: 6';
GO
