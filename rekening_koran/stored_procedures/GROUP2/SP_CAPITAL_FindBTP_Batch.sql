...
--
-- Description:
--   Find BTP (Bill To Party) for CAPITAL bank transactions (batch processing)
--   
-- Extraction Logic:
--   Group 2: Array[4] + Array[5] (Smart PT/CV)
--   Format: "KR OTOMATIS LLG-CAPITAL INDONE [WORD1] [WORD2] [WORD3]..."
--   
...
--
-- Pattern Count: 2 patterns in master data
--
-- Parameters:
...
CREATE OR ALTER PROCEDURE [dbo].[SP_CAPITAL_FindBTP_Batch]
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            SELECT 
                ...
                ROW_NUMBER() OVER (
                    ORDER BY 
                        CASE WHEN category = 'CAPITAL' THEN 1 ELSE 2 END,
            ...
            WHERE customer_name = @CustomerName
              AND (category = 'CAPITAL' OR category = 'NEW')
            ORDER BY 
                CASE WHEN category = 'CAPITAL' THEN 1 ELSE 2 END,
                match_percentage DESC,
                last_line_number DESC,
    ...
    PRINT '═══════════════════════════════════════════════════════════════════════';
    PRINT 'SP_CAPITAL_FindBTP_Batch - Processing Complete';
    PRINT '═══════════════════════════════════════════════════════════════════════';
...
PRINT '✅ SP_CAPITAL_FindBTP_Batch created successfully';
PRINT '   Bank: CAPITAL';
PRINT '   Group: 2 (Array[4] + Array[5])';
PRINT '   Special Word: INDONE';
PRINT '   Patterns in master: 2';
GO
