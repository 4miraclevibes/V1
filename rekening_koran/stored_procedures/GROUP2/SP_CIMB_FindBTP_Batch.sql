...
--
-- Description:
--   Find BTP (Bill To Party) for CIMB bank transactions (batch processing)
--   
-- Extraction Logic:
--   Group 2: Array[4] + Array[5] (Smart PT/CV)
--   Format: "KR OTOMATIS LLG-CIMB NIAGA [WORD1] [WORD2] [WORD3]..."
--   
--   Special Word: "NIAGA" at Array[3] (automatically skipped)
--   
--   Extraction:
--     • Array[4] + Array[5] (default: 2 words)
--     • If Array[4] = "PT" or "CV" → Array[4] + Array[5] + Array[6] (3 words)
--
-- Pattern Count: 97 patterns in master data
--
-- Parameters:
...
CREATE OR ALTER PROCEDURE [dbo].[SP_CIMB_FindBTP_Batch]
...
BEGIN
    ...
    BEGIN
        ...
        -- Special word "NIAGA" at Array[3] is skipped
        -- ═══════════════════════════════════════════════════════════════════
        ...
        BEGIN
            ...
            SELECT 
                ...
                ROW_NUMBER() OVER (
                    ORDER BY 
                        CASE WHEN category = 'CIMB' THEN 1 ELSE 2 END,
            ...
            WHERE customer_name = @CustomerName
              AND (category = 'CIMB' OR category = 'NEW')
            ORDER BY 
                CASE WHEN category = 'CIMB' THEN 1 ELSE 2 END,
                match_percentage DESC,
                last_line_number DESC,
    ...
    PRINT '═══════════════════════════════════════════════════════════════════════';
    PRINT 'SP_CIMB_FindBTP_Batch - Processing Complete';
    PRINT '═══════════════════════════════════════════════════════════════════════';
...
PRINT '✅ SP_CIMB_FindBTP_Batch created successfully';
PRINT '   Bank: CIMB';
PRINT '   Group: 2 (Array[4] + Array[5])';
PRINT '   Special Word: NIAGA';
PRINT '   Patterns in master: 97';
GO
