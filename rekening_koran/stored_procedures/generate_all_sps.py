#!/usr/bin/env python3
"""
Generate all bank stored procedures automatically
Based on extraction logic groups
"""
...
def generate_group1_sp(bank_name, pattern_count):
    """Generate stored procedure for Group 1 banks (Array[3] + Array[4])"""
    
    sp_content = f'''-- ═══════════════════════════════════════════════════════════════════════════
-- SP_{bank_name}_FindBTP_Batch
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Description:
--   Find BTP (Bill To Party) for {bank_name} bank transactions (batch processing)
--   
-- Extraction Logic:
--   Group 1: Array[3] + Array[4] (Smart PT/CV)
--   Format: "KR OTOMATIS LLG-{bank_name} [WORD1] [WORD2] [WORD3]..."
--   
--   Extraction:
--     • Array[3] + Array[4] (default: 2 words)
--     • If Array[3] = "PT" or "CV" → Array[3] + Array[4] + Array[5] (3 words)
--
-- Pattern Count: {pattern_count} patterns in master data
--
-- Parameters:
...
CREATE OR ALTER PROCEDURE [dbo].[SP_{bank_name}_FindBTP_Batch]
...
BEGIN
    ...
    BEGIN
        ...
        -- ═══════════════════════════════════════════════════════════════════
        -- Step 2: Extract customer name using Group 1 logic
        -- Format: "KR OTOMATIS LLG-{bank_name} [Word1] [Word2] [Word3]..."
        -- Array[3] + Array[4] (with PT/CV smart extraction)
        -- ═══════════════════════════════════════════════════════════════════
        ...
        BEGIN
            ...
            -- Array[3] in C = WordIndex 4 in SQL (SQL is 1-based, C is 0-based)
    ...
    PRINT '═══════════════════════════════════════════════════════════════════════';
    PRINT 'SP_{bank_name}_FindBTP_Batch - Processing Complete';
    PRINT '═══════════════════════════════════════════════════════════════════════';
...
PRINT '✅ SP_{bank_name}_FindBTP_Batch created successfully';
PRINT '   Bank: {bank_name}';
PRINT '   Group: 1 (Array[3] + Array[4])';
PRINT '   Patterns in master: {pattern_count}';
GO
'''
...
def generate_group2_sp(bank_name, special_word, pattern_count):
    """Generate stored procedure for Group 2 banks (Array[4] + Array[5])"""
...
--   Format: "KR OTOMATIS LLG-{bank_name} {special_word} [WORD1] [WORD2] [WORD3]..."
--   
--   Special Word: "{special_word}" at Array[3] (automatically skipped)
--   
--   Extraction:
...
BEGIN
    ...
    BEGIN
        ...
        -- Special word "{special_word}" at Array[3] is skipped
        -- ═══════════════════════════════════════════════════════════════════
...
PRINT '   Special Word: {special_word}';
PRINT '   Patterns in master: {pattern_count}';
GO
'''
...
def main():
    ...
    print("Generating stored procedures for all banks...")
    print("")
    
    # Generate Group 1 SPs
    print("Group 1: Array[3] + Array[4] - 9 banks")
    print("=" * 70)
    
    for bank_name, pattern_count in GROUP1_BANKS:
        filename = f"GROUP1/SP_{bank_name}_FindBTP_Batch.sql"
        content = generate_group1_sp(bank_name, pattern_count)
    ...
    # Generate Group 2 SPs
    print("Group 2: Array[4] + Array[5] - 9 banks")
    print("=" * 70)
    
    for bank_name, special_word, pattern_count in GROUP2_BANKS:
        filename = f"GROUP2/SP_{bank_name}_FindBTP_Batch.sql"
        content = generate_group2_sp(bank_name, special_word, pattern_count)
    ...
    print("")
    print("=" * 70)
    print(f"✅ Generated {len(GROUP1_BANKS) + len(GROUP2_BANKS)} stored procedures successfully!")
    print("")
    print("Next step: Create MASTER stored procedure for auto-routing")
