# SP_MANDIRI_FindBTP_Batch - Multiple Rows Feature

## 🎯 Your Request

> "kayak nya lebih bagus v1 deh, tapi kalau ditemukan 2 btp, tinggal return atau  
> munculkan data 2x, misalkan KOPERASI KARYAWAN itu memiliki 2 btp, return 2x  
> KOPERASI KARYAWAN nya, 1 dengan btp apa, persetase berapa bla bla bla, 1 nya  
...
## ✅ Solution: Updated SP_MANDIRI_FindBTP_Batch
...
**Input:** KOPERASI KARYAWAN (has 2 BTPs)

**Output:** 1 row only
```
TransactionID | CustomerName | BTP        | MatchPct | TotalBTPOptions | Message
TRX001       | KOPERASI KARYAWAN    | 2300015239 | 98.81    | 2              | Found 2 BTP options. Returning BEST.
```
...
TRX001       | KOPERASI KARYAWAN    | 2300015239 | 98.81    | 1           | YES      |            | BEST
TRX001       | KOPERASI KARYAWAN    | 2300015240 | 95.24    | 2           |          | YES        | LATEST
```
...
```json
[
    {"transaction_id": "T1", "description": "MANDIRI 001 RONNY YULIADY"},
    {"transaction_id": "T2", "description": "MANDIRI 002 KOPERASI KARYAWAN"},
    {"transaction_id": "T3", "description": "MANDIRI 003 HARDI PUTRA MUHARR"}
]
```

**Assumptions:**
- RONNY YULIADY has 1 BTP
- KOPERASI KARYAWAN has 2 BTPs
...
├──────┼──────────────────┼────────────┼───────┼────────┼─────────┼───────────┼─────────────────┤
│ T2   │ KOPERASI KARYAWAN        │ 2300015239 │ 98.81 │ 1      │ YES     │           │ BEST            │ ⬅️ Row 1 for T2
│ T2   │ KOPERASI KARYAWAN        │ 2300015240 │ 95.24 │ 2      │         │ YES       │ LATEST          │ ⬅️ Row 2 for T2
├──────┼──────────────────┼────────────┼───────┼────────┼─────────┼───────────┼─────────────────┤
...
DECLARE @JSON NVARCHAR(MAX) = N'[
    {"transaction_id": "1", "description": "MANDIRI 12345 KOPERASI KARYAWAN"}
]';
...
```sql
EXEC SP_MANDIRI_FindBTP_Batch_v2 @InputJSON = @JSON;
...
```sql
EXEC SP_MANDIRI_FindBTP_Batch @InputJSON = @JSON;
...
```csharp
var command = new SqlCommand("SP_MANDIRI_FindBTP_Batch", connection);
...
# Get all results
df = pd.read_sql(
    "EXEC SP_MANDIRI_FindBTP_Batch @InputJSON = ?", 
    conn, 
    params=[json_input]
)
...
ClearCollect(
    colBTPResults,
    SP_MANDIRI_FindBTP_Batch.Run({InputJSON: JSON(colTransactions)})
);
...
DECLARE @JSON NVARCHAR(MAX) = N'[
    {"transaction_id": "TEST", 
     "description": "MANDIRI E-BANKING CR 1304 683280.00 fresh milk KOPERASI KARYAWAN"}
]';

EXEC SP_MANDIRI_FindBTP_Batch @InputJSON = @JSON, @Debug = 1;

-- Expected: 2 rows if KOPERASI KARYAWAN has 2 BTPs
