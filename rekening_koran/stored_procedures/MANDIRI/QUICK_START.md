# 🚀 MANDIRI BTP Matching - Quick Start Guide
...
```sql
-- Run in order:
1. SP_MANDIRI_FindBTP_Single.sql    -- Single search
2. SP_MANDIRI_FindBTP_Batch.sql     -- Batch search (returns multiple rows for multiple BTPs)
```
...
EXEC [dbo].[SP_MANDIRI_FindBTP_Single] 
    @Description = 'MANDIRI E-BANKING 12345 KOPERASI KARYAWAN';
```
...
EXEC [dbo].[SP_MANDIRI_FindBTP_Single] 
    @Description = 'MANDIRI FROM BCA 123456789 100000 RONNY YULIADY',
    @Debug = 0;
...
DECLARE @BatchJSON NVARCHAR(MAX) = N'[
    {"transaction_id": "INV001", "description": "MANDIRI 12345 RONNY YULIADY"},
    {"transaction_id": "INV002", "description": "MANDIRI 67890 KOPERASI KARYAWAN"},
    ... // 98 more
]';
...
DECLARE @BatchJSON NVARCHAR(MAX) = N'[
    {"transaction_id": "INV001", "description": "MANDIRI 12345 CHRISTIAN"},
    {"transaction_id": "INV002", "description": "MANDIRI 67890 RONNY YULIADY"}
]';
...
EXEC [dbo].[SP_MANDIRI_FindBTP_Batch] @InputJSON = @UnmatchedJSON;
...
```json
[
    {"description": "MANDIRI 12345 CUSTOMER NAME"}
]
```
...
[
    {
        ...
        "description": "MANDIRI E-BANKING CR 12345 100000 CUSTOMER NAME"
    }
]
```
...
public class BTPMatcher
{
    public async Task<List<BTPResult>> FindBTPsBatch(List<Transaction> transactions)
    {
        ...
        using (var cmd = new SqlCommand("[dbo].[SP_MANDIRI_FindBTP_Batch]", conn))
        {
...
transactions = [
    ...
    {"transaction_id": "T2", "description": "MANDIRI 67890 HARDI PUTRA"}
]
...
async function findBTPsBatch(transactions) {
    ...
    const result = await pool.request()
        ...
        .execute('[dbo].[SP_MANDIRI_FindBTP_Batch]');
...
2. **Run tests**: `Test_SP_MANDIRI.sql`
...
- [ ] Master data imported (130 patterns)
- [ ] SP_MANDIRI_FindBTP_Single created
- [ ] SP_MANDIRI_FindBTP_Batch created (returns multiple rows for multiple BTPs)
...
**Quick Example:**
```sql
EXEC SP_MANDIRI_FindBTP_Batch @InputJSON = N'[
    {"transaction_id": "1", "description": "MANDIRI 12345 CHRISTIAN"}
]';
