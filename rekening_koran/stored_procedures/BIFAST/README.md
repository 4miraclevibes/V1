# BI-FAST BTP Pattern Matching - Stored Procedures

## 📋 Overview

Stored procedures untuk mencari BTP (Bill To Party) dari deskripsi transaksi BI-FAST berdasarkan customer name pattern matching.
...
| File | Purpose |
|------|---------|
| `SP_BI-FAST_FindBTP_Single.sql` | Single search - 1 description → 1 BTP |
| `SP_BI-FAST_FindBTP_Batch.sql` | Batch search - Multiple descriptions (JSON) → Multiple BTPs |
| `Test_SP_BI-FAST.sql` | Test scripts dengan sample data |
...
2. Table must have column `category` = 'BI-FAST'
3. SQL Server 2016+ (for JSON support)

### Install:
```sql
-- 1. Create table (if not exists)
-- Run: pattern_generator_BI-FAST/master_customer_btp_pattern_BI-FAST_SPLIT.sql

-- 2. Create SP Single
-- Run: SP_BI-FAST_FindBTP_Single.sql

-- 3. Create SP Batch
-- Run: SP_BI-FAST_FindBTP_Batch.sql

-- 4. Test
-- Run: Test_SP_BI-FAST.sql
```
...
**Example**:
```sql
EXEC [dbo].[SP_BI-FAST_FindBTP_Single] 
    @Description = 'BI-FAST E-BANKING CR 0201/FTSCY/WS95011 455520.00 RONNY YULIADY',
    @Debug = 0;
```
...
[
    {
        ...
        "description": "BI-FAST E-BANKING CR 0201 455520.00 RONNY YULIADY"
    },
    {
        "transaction_id": "TRX002",
        "description": "BI-FAST FROM BCA 123456789 HARDI PUTRA MUHARR"
    }
]
```
...
EXEC [dbo].[SP_BI-FAST_FindBTP_Batch] 
    @InputJSON = @JSON,
    @Debug = 0;
```
...
--------------|-------------------|-------------------|------------|----------|-----------|------------------
TRX001        | BI-FAST E-BANKING... | RONNY YULIADY     | 2300016055 | 95.5     | EXCELLENT | High confidence
TRX002        | BI-FAST FROM BCA...  | HARDI PUTRA MUHARR| 2300016055 | 100.0    | EXCELLENT | High confidence
```
...
-- Test Batch
DECLARE @JSON NVARCHAR(MAX) = N'[
    {"transaction_id": "T1", "description": "BI-FAST 12345 RONNY YULIADY"},
    {"transaction_id": "T2", "description": "BI-FAST 67890 HARDI PUTRA MUHARR"}
]';

EXEC [dbo].[SP_BI-FAST_FindBTP_Batch] @InputJSON = @JSON, @Debug = 1;
```

### Run Full Test Suite:
```sql
-- See: Test_SP_BI-FAST.sql
```
...
cd pattern_generator_BI-FAST
./generate_patterns_bifast

-- 2. Truncate existing data
TRUNCATE TABLE [dbo].[MASTER_CUSTOMER_BTP_PATTERN];

-- 3. Import new patterns
-- Run: master_customer_btp_pattern_BI-FAST_SPLIT.sql
```
...
EXEC [dbo].[SP_BI-FAST_FindBTP_Single] 
    @Description = 'BI-FAST 12345 TEST CUSTOMER';
```
...
1. Check test results in `Test_SP_BI-FAST.sql`
