# 🚀 BI-FAST BTP Matching - Quick Start Guide
...
```sql
-- Run in order:
1. SP_BI-FAST_FindBTP_Single.sql    -- Single search
2. SP_BI-FAST_FindBTP_Batch.sql     -- Batch search (returns multiple rows for multiple BTPs)
```
...
EXEC [dbo].[SP_BI-FAST_FindBTP_Single] 
    @Description = 'BI-FAST E-BANKING 12345 WIDYAN AUFAR';
```
...
EXEC [dbo].[SP_BI-FAST_FindBTP_Single] 
    @Description = 'BI-FAST FROM BCA 123456789 100000 RONNY YULIADY',
    @Debug = 0;
...
### Use Case 2: Batch Import dari File (Simple)
```sql
-- Import 100 transactions dari file CSV/Excel
-- Convert to JSON format
DECLARE @BatchJSON NVARCHAR(MAX) = N'[
    {"transaction_id": "INV001", "description": "BI-FAST 12345 RONNY YULIADY"},
    {"transaction_id": "INV002", "description": "BI-FAST 67890 WIDYAN AUFAR"},
    ... // 98 more
]';
...
DECLARE @BatchJSON NVARCHAR(MAX) = N'[
    {"transaction_id": "INV001", "description": "BI-FAST 12345 CHRISTIAN"},
    {"transaction_id": "INV002", "description": "BI-FAST 67890 RONNY YULIADY"}
]';
...
EXEC [dbo].[SP_BI-FAST_FindBTP_Batch] @InputJSON = @UnmatchedJSON;
...
```json
[
    {"description": "BI-FAST 12345 CUSTOMER NAME"}
]
```
...
[
    {
        ...
        "description": "BI-FAST E-BANKING CR 12345 100000 CUSTOMER NAME"
    }
]
```
...
public class BTPMatcher
{
    public async Task<List<BTPResult>> FindBTPsBatch(List<Transaction> transactions)
    {
        ...
        using (var cmd = new SqlCommand("[dbo].[SP_BI-FAST_FindBTP_Batch]", conn))
        {
...
transactions = [
    ...
    {"transaction_id": "T2", "description": "BI-FAST 67890 HARDI PUTRA"}
]
...
async function findBTPsBatch(transactions) {
    ...
    const result = await pool.request()
        ...
        .execute('[dbo].[SP_BI-FAST_FindBTP_Batch]');
...
2. **Run tests**: `Test_SP_BI-FAST.sql`
...
- [ ] Master data imported (763 patterns)
- [ ] SP_BI-FAST_FindBTP_Single created
- [ ] SP_BI-FAST_FindBTP_Batch created (returns multiple rows for multiple BTPs)
...
EXEC SP_BI-FAST_FindBTP_Batch @InputJSON = N'[
    {"transaction_id": "1", "description": "BI-FAST 12345 CHRISTIAN"}
]';
