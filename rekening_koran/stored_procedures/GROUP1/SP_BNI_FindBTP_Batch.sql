...
--
-- Description:
--   Find BTP (Bill To Party) for BNI bank transactions (batch processing)
--   
-- Extraction Logic:
--   Group 1: Array[3] + Array[4] (Smart PT/CV)
--   Format: "KR OTOMATIS LLG-BNI [WORD1] [WORD2] [WORD3]..."
--   
--   Extraction:
--     • Array[3] + Array[4] (default: 2 words)
--     • If Array[3] = "PT" or "CV" → Array[3] + Array[4] + Array[5] (3 words)
--
-- Pattern Count: 19 patterns in master data
--
-- Parameters:
...
CREATE OR ALTER PROCEDURE [dbo].[SP_BNI_FindBTP_Batch]
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            SELECT 
                ...
                ROW_NUMBER() OVER (
                    ORDER BY 
                        CASE WHEN category = 'BNI' THEN 1 ELSE 2 END,
            ...
            WHERE customer_name = @CustomerName
              AND (category = 'BNI' OR category = 'NEW')
            ORDER BY 
                CASE WHEN category = 'BNI' THEN 1 ELSE 2 END,
                match_percentage DESC,
                last_line_number DESC,
    ...
    PRINT '═══════════════════════════════════════════════════════════════════════';
    PRINT 'SP_BNI_FindBTP_Batch - Processing Complete';
    PRINT '═══════════════════════════════════════════════════════════════════════';
...
PRINT '✅ SP_BNI_FindBTP_Batch created successfully';
PRINT '   Bank: BNI';
PRINT '   Group: 1 (Array[3] + Array[4])';
PRINT '   Patterns in master: 19';
GO
