...
int main() {
    printf("Starting SINARMAS pattern generation...\n");
    
    FILE* file = fopen("SINARMAS.csv", "r");
    if (!file) {
        printf("Error: Cannot open SINARMAS.csv\n");
        return 1;
    }
    ...
    printf("Processing SINARMAS data...\n");
    ...
    // Generate SQL file
    FILE* sql_file = fopen("master_customer_btp_pattern_SINARMAS.sql", "w");
    if (!sql_file) {
        printf("Error: Cannot create SQL file\n");
        return 1;
    }
    
    fprintf(sql_file, "-- SINARMAS Customer BTP Patterns\n");
    fprintf(sql_file, "-- Generated from SINARMAS.csv\n");
    ...
    for (int i = 0; i < pattern_count; i++) {
        ...
        fprintf(sql_file, "('%s', '%s', \'SINARMAS\', %d, %d, %.2f, %d)",
