...
int main() {
    printf("Starting BRI pattern generation...\n");
    
    FILE* file = fopen("BRI.csv", "r");
    if (!file) {
        printf("Error: Cannot open BRI.csv\n");
        return 1;
    }
    ...
    // Generate SQL file
    FILE* sql_file = fopen("master_customer_btp_pattern_BRI.sql", "w");
    if (!sql_file) {
        printf("Error: Cannot create SQL file\n");
        return 1;
    }
    
    fprintf(sql_file, "-- BRI Customer BTP Patterns\n");
    fprintf(sql_file, "-- Generated from BRI.csv\n");
    ...
    for (int i = 0; i < pattern_count; i++) {
        ...
        fprintf(sql_file, "('%s', '%s', \'BRI\', %d, %d, %.2f, %d)",
