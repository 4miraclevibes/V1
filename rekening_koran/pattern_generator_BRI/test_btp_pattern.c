...
// Function to extract customer name from BRI description
// Format: "KR OTOMATIS LLG-BRI [CUSTOMER_WORD1] [CUSTOMER_WORD2] [ADDITIONAL_INFO]"
...
int load_patterns() {
    FILE* file = fopen("master_customer_btp_pattern_BRI.sql", "r");
    if (!file) {
        printf("Error: Cannot open master_customer_btp_pattern_BRI.sql\n");
        return 0;
    }
...
void manual_test() {
    ...
    printf("\n=== MANUAL TEST BRI PATTERN ===\n");
    printf("Masukkan description BRI (format: KR OTOMATIS LLG-BRI ...):\n");
    printf("Contoh: KR OTOMATIS LLG-BRI KARYAWAN ISS SUKSE Pembeian Susu Gree nfield 3Crt KISSCa fe - 231226\n");
    ...
    if (fgets(description, sizeof(description), stdin)) {
        ...
        if (extract_customer_name(description, customer_name)) {
            ...
            printf("Format yang benar: KR OTOMATIS LLG-BRI [WORD1] [WORD2] [ADDITIONAL_INFO]\n");
        }
    }
}
...
int main() {
    printf("BRI BTP Pattern Tester\n");
    printf("======================\n");
    
    if (!load_patterns()) {
        printf("Failed to load patterns. Make sure master_customer_btp_pattern_BRI.sql exists.\n");
        return 1;
    }
    
    printf("Loaded %d patterns from master_customer_btp_pattern_BRI.sql\n", pattern_count);
