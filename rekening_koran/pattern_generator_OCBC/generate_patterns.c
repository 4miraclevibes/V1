...
// Format: "KR OTOMATIS LLG-OCBC NISP [CUSTOMER_WORD1] [CUSTOMER_WORD2] [ADDITIONAL_INFO]"
...
int main() {
    printf("Starting OCBC pattern generation...\n");
    
    FILE* file = fopen("OCBC.csv", "r");
    if (!file) {
        printf("Error: Cannot open OCBC.csv\n");
        return 1;
    }
    ...
    // Generate SQL file
    FILE* sql_file = fopen("master_customer_btp_pattern_OCBC.sql", "w");
    if (!sql_file) {
        printf("Error: Cannot create SQL file\n");
        return 1;
    }
    
    fprintf(sql_file, "-- OCBC Customer BTP Patterns\n");
    fprintf(sql_file, "-- Generated from OCBC.csv\n");
    ...
    for (int i = 0; i < pattern_count; i++) {
        ...
        fprintf(sql_file, "('%s', '%s', \'OCBC\', %d, %d, %.2f, %d)",
