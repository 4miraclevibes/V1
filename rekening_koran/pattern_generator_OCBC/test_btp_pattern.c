...
// Function to extract customer name from OCBC description
// Format: "KR OTOMATIS LLG-OCBC NISPESIA [CUSTOMER_WORD1] [CUSTOMER_WORD2] [ADDITIONAL_INFO]"
...
int load_patterns() {
    FILE* file = fopen("master_customer_btp_pattern_OCBC.sql", "r");
    if (!file) {
        printf("Error: Cannot open master_customer_btp_pattern_OCBC.sql\n");
        return 0;
    }
...
void manual_test() {
    ...
    printf("\n=== MANUAL TEST OCBC PATTERN ===\n");
    printf("Masukkan description OCBC (format: KR OTOMATIS LLG-OCBC NISPESIA ...):\n");
    printf("Contoh: KR OTOMATIS LLG-OCBC NISPESIA EMPU ANEKA PANGAN, GREENFIELDS\n");
    ...
    if (fgets(description, sizeof(description), stdin)) {
        ...
        if (extract_customer_name(description, customer_name)) {
            ...
            printf("Format yang benar: KR OTOMATIS LLG-OCBC NISPESIA [WORD1] [WORD2] [ADDITIONAL_INFO]\n");
        }
    }
}
...
int main() {
    printf("OCBC BTP Pattern Tester\n");
    printf("======================\n");
    
    if (!load_patterns()) {
        printf("Failed to load patterns. Make sure master_customer_btp_pattern_OCBC.sql exists.\n");
        return 1;
    }
    
    printf("Loaded %d patterns from master_customer_btp_pattern_OCBC.sql\n", pattern_count);
