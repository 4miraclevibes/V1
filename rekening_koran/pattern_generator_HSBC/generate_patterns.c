...
int main() {
    printf("Starting HSBC pattern generation...\n");
    
    FILE* file = fopen("HSBC.csv", "r");
    if (!file) {
        printf("Error: Cannot open HSBC.csv\n");
        return 1;
    }
    ...
    // Generate SQL file
    FILE* sql_file = fopen("master_customer_btp_pattern_HSBC.sql", "w");
    if (!sql_file) {
        printf("Error: Cannot create SQL file\n");
        return 1;
    }
    
    fprintf(sql_file, "-- HSBC Customer BTP Patterns\n");
    fprintf(sql_file, "-- Generated from HSBC.csv\n");
    ...
    for (int i = 0; i < pattern_count; i++) {
        ...
        fprintf(sql_file, "('%s', '%s', \'HSBC\', %d, %d, %.2f, %d)",
