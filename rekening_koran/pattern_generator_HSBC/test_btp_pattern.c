...
// Function to extract customer name from HSBC description
// Format: "KR OTOMATIS LLG-HSBC INDONESIA [CUSTOMER_WORD1] [CUSTOMER_WORD2] [ADDITIONAL_INFO]"
...
int load_patterns() {
    FILE* file = fopen("master_customer_btp_pattern_HSBC.sql", "r");
    if (!file) {
        printf("Error: Cannot open master_customer_btp_pattern_HSBC.sql\n");
        return 0;
    }
...
void manual_test() {
    ...
    printf("\n=== MANUAL TEST HSBC PATTERN ===\n");
    printf("Masukkan description HSBC (format: KR OTOMATIS LLG-HSBC INDONESIA ...):\n");
    printf("Contoh: KR OTOMATIS LLG-HSBC INDONESIA PT AMERTA INDAH OT 3300000508\n");
    ...
    if (fgets(description, sizeof(description), stdin)) {
        ...
        if (extract_customer_name(description, customer_name)) {
            ...
            printf("Format yang benar: KR OTOMATIS LLG-HSBC INDONESIA [WORD1] [WORD2] [ADDITIONAL_INFO]\n");
        }
    }
}
...
int main() {
    printf("HSBC BTP Pattern Tester\n");
    printf("=======================\n");
    
    if (!load_patterns()) {
        printf("Failed to load patterns. Make sure master_customer_btp_pattern_HSBC.sql exists.\n");
        return 1;
    }
    
    printf("Loaded %d patterns from master_customer_btp_pattern_HSBC.sql\n", pattern_count);
