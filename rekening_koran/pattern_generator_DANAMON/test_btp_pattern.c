...
int load_patterns() {
    FILE* file = fopen("master_customer_btp_pattern_DANAMON.sql", "r");
    if (!file) {
        printf("Error: Cannot open master_customer_btp_pattern_DANAMON.sql\n");
        return 0;
    }
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        // Look for lines with INSERT INTO ... VALUES
        if (strstr(line, "INSERT INTO") != NULL && strstr(line, "VALUES") != NULL) {
...
void manual_test() {
    ...
    printf("\n=== MANUAL TEST DANAMON PATTERN ===\n");
    printf("Masukkan description DANAMON (format: KR OTOMATIS LLG-DANAMON ...):\n");
    printf("Contoh: KR OTOMATIS LLG-DANAMON PT TRISULA ADI TEGUH BUKTI BAYAR\n");
        ...
        if (extract_customer_name(description, customer_name)) {
            ...
            if (find_all_btps(customer_name, found_patterns, &found_count)) {
                ...
                printf("- Ketik 'B' untuk melihat BTP terbaru\n");
                   ...
                   printf("Format yang benar: KR OTOMATIS LLG-DANAMON [WORD1] [WORD2] [ADDITIONAL_INFO]\n");
        }
    }
}
...
int main() {
    printf("DANAMON BTP Pattern Tester\n");
    printf("==========================\n");
    
    if (!load_patterns()) {
        printf("Failed to load patterns. Make sure master_customer_btp_pattern_DANAMON.sql exists.\n");
        return 1;
    }
    
    printf("Loaded %d patterns from master_customer_btp_pattern_DANAMON.sql\n", pattern_count);
