...
int main() {
    printf("Starting MEGA pattern generation...\n");
    
    FILE* file = fopen("MEGA.csv", "r");
    if (!file) {
        printf("Error: Cannot open MEGA.csv\n");
        return 1;
    }
    ...
    // Generate SQL file
    FILE* sql_file = fopen("master_customer_btp_pattern_MEGA.sql", "w");
    if (!sql_file) {
        printf("Error: Cannot create SQL file\n");
        return 1;
    }
    
    fprintf(sql_file, "-- MEGA Customer BTP Patterns\n");
    fprintf(sql_file, "-- Generated from MEGA.csv\n");
    ...
    for (int i = 0; i < pattern_count; i++) {
        ...
        fprintf(sql_file, "('%s', '%s', \'MEGA\', %d, %d, %.2f, %d)",
