...
// Function to extract customer name from MEGA description
// Format: "KR OTOMATIS LLG-MEGA [CUSTOMER_WORD1] [CUSTOMER_WORD2] [ADDITIONAL_INFO]"
...
int load_patterns() {
    FILE* file = fopen("master_customer_btp_pattern_MEGA.sql", "r");
    if (!file) {
        printf("Error: Cannot open master_customer_btp_pattern_MEGA.sql\n");
        return 0;
    }
...
void manual_test() {
    ...
    printf("\n=== MANUAL TEST MEGA PATTERN ===\n");
    printf("Masukkan description MEGA (format: KR OTOMATIS LLG-MEGA ...):\n");
    printf("Contoh: KR OTOMATIS LLG-MEGA SUSPEN HUB CIKINI BYR KONSUMSI TRANS FER/22/SS/24/01/01 4\n");
    ...
    if (fgets(description, sizeof(description), stdin)) {
        ...
        if (extract_customer_name(description, customer_name)) {
            ...
            printf("Format yang benar: KR OTOMATIS LLG-MEGA [WORD1] [WORD2] [ADDITIONAL_INFO]\n");
        }
    }
}
...
int main() {
    printf("MEGA BTP Pattern Tester\n");
    printf("======================\n");
    
    if (!load_patterns()) {
        printf("Failed to load patterns. Make sure master_customer_btp_pattern_MEGA.sql exists.\n");
        return 1;
    }
    
    printf("Loaded %d patterns from master_customer_btp_pattern_MEGA.sql\n", pattern_count);
