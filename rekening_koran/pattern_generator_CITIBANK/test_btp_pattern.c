...
// Function to extract customer name from CITIBANK description
// Format: "KR OTOMATIS LLG-CITIBANK [CUSTOMER_WORD1] [CUSTOMER_WORD2] [ADDITIONAL_INFO]"
...
int load_patterns() {
    FILE* file = fopen("master_customer_btp_pattern_CITIBANK.sql", "r");
    if (!file) {
        printf("Error: Cannot open master_customer_btp_pattern_CITIBANK.sql\n");
        return 0;
    }
...
void manual_test() {
    ...
    printf("\n=== MANUAL TEST CITIBANK PATTERN ===\n");
    printf("Masukkan description CITIBANK (format: KR OTOMATIS LLG-CITIBANK ...):\n");
    printf("Contoh: KR OTOMATIS LLG-CITIBANK KARYAWAN ISS SUKSE Pembeian Susu Gree nfield 3Crt KISSCa fe - 231226\n");
    ...
    if (fgets(description, sizeof(description), stdin)) {
        ...
        if (extract_customer_name(description, customer_name)) {
            ...
            printf("Format yang benar: KR OTOMATIS LLG-CITIBANK [WORD1] [WORD2] [ADDITIONAL_INFO]\n");
        }
    }
}
...
int main() {
    printf("CITIBANK BTP Pattern Tester\n");
    printf("======================\n");
    
    if (!load_patterns()) {
        printf("Failed to load patterns. Make sure master_customer_btp_pattern_CITIBANK.sql exists.\n");
        return 1;
    }
    
    printf("Loaded %d patterns from master_customer_btp_pattern_CITIBANK.sql\n", pattern_count);
