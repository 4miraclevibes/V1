...
int main() {
    printf("Starting CITIBANK pattern generation...\n");
    
    FILE* file = fopen("CITIBANK.csv", "r");
    if (!file) {
        printf("Error: Cannot open CITIBANK.csv\n");
        return 1;
    }
    ...
    printf("Processing CITIBANK data...\n");
    ...
    printf("Skipped %d lines (invalid format)\n", skipped_lines);
    ...
    // Generate SQL file
    FILE* sql_file = fopen("master_customer_btp_pattern_CITIBANK.sql", "w");
    if (!sql_file) {
        printf("Error: Cannot create SQL file\n");
        return 1;
    }
    
    fprintf(sql_file, "-- CITIBANK Customer BTP Patterns\n");
    fprintf(sql_file, "-- Generated from CITIBANK.csv\n");
    fprintf(sql_file, "-- Total patterns: %d\n", pattern_count);
    fprintf(sql_file, "-- Skipped invalid format: %d lines\n\n", skipped_lines);
    ...
    for (int i = 0; i < pattern_count; i++) {
        ...
        fprintf(sql_file, "('%s', '%s', \'CITIBANK\', %d, %d, %.2f, %d)",
    ...
    // Print summary
    printf("\n=== CITIBANK PATTERN GENERATION COMPLETE ===\n");
    printf("Total patterns generated: %d\n", pattern_count);
    printf("Total unique customers: %d\n", customer_total_count);
    printf("Skipped invalid format: %d lines\n", skipped_lines);
    printf("SQL file created: master_customer_btp_pattern_CITIBANK.sql\n\n");
