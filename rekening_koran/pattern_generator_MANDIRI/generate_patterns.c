...
int main() {
    printf("Starting MANDIRI pattern generation...\n");
    
    FILE* file = fopen("MANDIRI.csv", "r");
    if (!file) {
        printf("Error: Cannot open MANDIRI.csv\n");
        return 1;
    }
    ...
    printf("Processing MANDIRI data...\n");
    ...
    // Generate SQL file
    FILE* sql_file = fopen("master_customer_btp_pattern_MANDIRI.sql", "w");
    if (!sql_file) {
        printf("Error: Cannot create SQL file\n");
        return 1;
    }
    
    fprintf(sql_file, "-- MANDIRI Customer BTP Patterns\n");
    fprintf(sql_file, "-- Generated from MANDIRI.csv\n");
    ...
    for (int i = 0; i < pattern_count; i++) {
        ...
        fprintf(sql_file, "('%s', '%s', \'MANDIRI\', %d, %d, %.2f, %d)",
    ...
    // Print summary
    printf("\n=== MANDIRI PATTERN GENERATION COMPLETE ===\n");
    printf("Total patterns generated: %d\n", pattern_count);
    printf("Total unique customers: %d\n", customer_total_count);
    printf("SQL file created: master_customer_btp_pattern_MANDIRI.sql\n\n");
