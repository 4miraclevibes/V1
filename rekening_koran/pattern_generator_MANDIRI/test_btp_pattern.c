...
// Function to extract customer name from MANDIRI description
// Format: "KR OTOMATIS LLG-MANDIRI [CUSTOMER_WORD1] [CUSTOMER_WORD2] [ADDITIONAL_INFO]"
...
int load_patterns() {
    FILE* file = fopen("master_customer_btp_pattern_MANDIRI.sql", "r");
    if (!file) {
        printf("Error: Cannot open master_customer_btp_pattern_MANDIRI.sql\n");
        return 0;
    }
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (in_values && strstr(line, "(") != NULL) {
            ...
            if (start) {
                ...
                char* customer_name = strtok(start, ",");
                char* btp = strtok(NULL, ",");
...
void manual_test() {
    ...
    printf("\n=== MANUAL TEST MANDIRI PATTERN ===\n");
    printf("Masukkan description MANDIRI (format: KR OTOMATIS LLG-MANDIRI ...):\n");
    printf("Contoh: KR OTOMATIS LLG-MANDIRI SMART FOOD CONCEPT -0053061777 Clearing947991\n");
    ...
    if (fgets(description, sizeof(description), stdin)) {
        ...
        if (extract_customer_name(description, customer_name)) {
            ...
            printf("Format yang benar: KR OTOMATIS LLG-MANDIRI [WORD1] [WORD2] [ADDITIONAL_INFO]\n");
        }
    }
}
...
int main() {
    printf("MANDIRI BTP Pattern Tester\n");
    printf("==========================\n");
    
    if (!load_patterns()) {
        printf("Failed to load patterns. Make sure master_customer_btp_pattern_MANDIRI.sql exists.\n");
        return 1;
    }
    
    printf("Loaded %d patterns from master_customer_btp_pattern_MANDIRI.sql\n", pattern_count);
