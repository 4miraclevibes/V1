-- =====================================================
-- update_accountCap_Accounts.sql
-- =====================================================
-- Update [accountCap] di Accounts berdasarkan accountCap.csv
-- Match: Accounts.Account = account_name (CSV)
--
-- Prasyarat: ALTER_ADD_accountCap_Accounts.sql
-- Sumber: mp_customer/accountCap.csv (219 rows)
-- =====================================================
...
BEGIN TRY
    ...
    BEGIN
        SELECT
            a.[Id],
            a.[Account],
            a.[accountCap] AS Current_accountCap,
            s.[accountCap] AS New_accountCap
        FROM [dbo].[Accounts] a
        INNER JOIN @Source s
        ...
        SELECT s.[account_name], s.[accountCap]
        FROM @Source s
        WHERE NOT EXISTS (
            SELECT 1
            FROM [dbo].[Accounts] a
            WHERE LTRIM(RTRIM(ISNULL(a.[Account], ''))) COLLATE Latin1_General_CI_AI
                = LTRIM(RTRIM(s.[account_name])) COLLATE Latin1_General_CI_AI
        )
    ...
    SELECT
        ...
        ) AS AccountsWithAccountCapAfter,
        (
            SELECT COUNT(*)
            FROM @Source s
            WHERE NOT EXISTS (
        ...
        ) AS CsvAccountNamesNotFoundInAccounts,
        GETDATE() AS UpdatedAt,
        N'Update accountCap Accounts berhasil' AS Message;
END TRY
BEGIN CATCH
    IF @@TRANCOUNT > 0
    ...
    SELECT 0 AS RowsUpdated, @ErrorMessage AS ErrorMessage;
