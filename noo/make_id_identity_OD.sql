-- Script untuk membuat kolom id menjadi IDENTITY (auto increment) - MP_FSS_OD_NEW
USE [POWERAPPS]
GO

PRINT 'Mengecek status tabel MP_FSS_OD_NEW...'

-- Cek apakah kolom id sudah IDENTITY
IF EXISTS (SELECT * FROM sys.identity_columns WHERE object_id = OBJECT_ID('MP_FSS_OD_NEW') AND name = 'id')
BEGIN
...
ALTER TABLE [dbo].[MP_FSS_OD_NEW] ADD id_backup BIGINT
PRINT 'Kolom backup ditambahkan'
GO

-- Copy data id ke backup
UPDATE [dbo].[MP_FSS_OD_NEW] SET id_backup = id
...
ALTER TABLE [dbo].[MP_FSS_OD_NEW] ADD id BIGINT IDENTITY(1,1) NOT NULL
PRINT 'Kolom id baru ditambahkan sebagai IDENTITY'
GO

-- Copy data dari backup ke id baru
UPDATE [dbo].[MP_FSS_OD_NEW] SET id = id_backup
PRINT 'Data di-restore ke id baru'
GO

-- Hapus kolom backup
ALTER TABLE [dbo].[MP_FSS_OD_NEW] DROP COLUMN id_backup
PRINT 'Kolom backup dihapus'
GO

-- Set Primary Key
ALTER TABLE [dbo].[MP_FSS_OD_NEW] ADD CONSTRAINT PK_MP_FSS_OD_NEW PRIMARY KEY (id)
